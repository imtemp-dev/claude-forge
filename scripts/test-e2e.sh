...
mkdir -p .forge/state/recipes/test-001
echo '{"id":"test-001","type":"blueprint","topic":"OAuth2","phase":"verify","iteration":1,"level":1.5,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/test-001/recipe.json
$FORGE recipe log test-001 --iteration 1 --critical 2 --major 1 > /dev/null
[ -f .forge/state/recipes/test-001/verify-log.jsonl ] && echo "✓ 3. recipe log (verify-log)" || { echo "✗ 3. verify-log"; exit 1; }

# 4. Recipe log (changelog action)
$FORGE recipe log test-001 --action improve --output draft.md > /dev/null
[ -f .forge/state/recipes/test-001/changelog.jsonl ] && echo "✓ 4. recipe log (changelog)" || { echo "✗ 4. changelog"; exit 1; }

# 5. Recipe log (manifest update)
$FORGE recipe log test-001 --action research --output research/v1.md --based-on "topic" > /dev/null
[ -f .forge/state/recipes/test-001/manifest.json ] && echo "✓ 5. recipe log (manifest)" || { echo "✗ 5. manifest"; exit 1; }
...
echo '{"iteration":2,"critical":0,"major":0,"minor":1,"status":"converged","timestamp":"2026-03-18T00:01:00Z"}' >> .forge/state/recipes/test-001/verify-log.jsonl
echo "# Verification findings" > .forge/state/recipes/test-001/verification.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"stop","content":"<forge>DONE</forge>"}' | $FORGE hook stop 2>&1; echo "EXIT:$?")
echo "$RESULT" | grep -q "EXIT:0" && echo "✓ 12. stop hook allows (converged)" || { echo "✗ 12. stop allow"; exit 1; }

# --- IMPLEMENT DONE stop hook tests ---
mkdir -p .forge/state/recipes/impl-001
echo '{"id":"impl-001","type":"blueprint","topic":"Auth","phase":"status","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/impl-001/recipe.json
echo '{"recipe_id":"impl-001","started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z","tasks":[{"id":"t-001","file":"src/auth.ts","action":"create","status":"done","description":"auth","depends_on":[],"retry_count":0,"last_error":""}]}' > .forge/state/recipes/impl-001/tasks.json
echo '{"recipe_id":"impl-001","run_at":"2026-03-18T00:00:00Z","framework":"jest","iterations":1,"status":"pass","total":5,"passed":5,"failed":0,"skipped":0}' > .forge/state/recipes/impl-001/test-results.json
echo "# Deviation Report" > .forge/state/recipes/impl-001/deviation.md
...
echo "# Code Review" > .forge/state/recipes/impl-001/review.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"stop","content":"<forge>IMPLEMENT DONE</forge>"}' | $FORGE hook stop 2>&1; echo "EXIT:$?")
...
mkdir -p .forge/state/recipes/fix-001
echo '{"id":"fix-001","type":"fix","topic":"Login bug","phase":"test","iteration":1,"level":0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z","ref_recipe":"test-001"}' > .forge/state/recipes/fix-001/recipe.json

# 15. FIX DONE — BLOCK (no fix-spec.md)
echo '{"recipe_id":"fix-001","run_at":"2026-03-18T00:00:00Z","framework":"jest","iterations":1,"status":"pass","total":3,"passed":3,"failed":0,"skipped":0}' > .forge/state/recipes/fix-001/test-results.json
...
echo "# Fix Spec" > .forge/state/recipes/fix-001/fix-spec.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"stop","content":"<forge>FIX DONE</forge>"}' | $FORGE hook stop 2>&1; echo "EXIT:$?")
echo "$RESULT" | grep -q "EXIT:0" && echo "✓ 16. FIX DONE allows (spec + tests pass)" || { echo "✗ 16. FIX DONE allow"; exit 1; }

# --- Session-start hook tests ---
echo '{"id":"impl-001","type":"blueprint","topic":"Auth","phase":"complete","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/impl-001/recipe.json
echo '{"id":"fix-001","type":"fix","topic":"Login bug","phase":"complete","iteration":1,"level":0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/fix-001/recipe.json

# 17. Session-start — review phase → /forge-implement hint
mkdir -p .forge/state/recipes/ss-001
echo '{"id":"ss-001","type":"blueprint","topic":"API","phase":"review","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/ss-001/recipe.json
...
echo '{"id":"ss-001","type":"blueprint","topic":"API","phase":"finalize","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/ss-001/recipe.json
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"session_start"}' | $FORGE hook session-start 2>&1)
...
echo '{"current_draft":"draft.md","level":2.0,"documents":{"review.md":{"type":"review","created_at":"2026-03-18T00:00:00Z"}}}' > .forge/state/recipes/ss-001/manifest.json
...
echo '{"id":"test-001","type":"blueprint","topic":"OAuth2","phase":"cancelled","iteration":1,"level":1.5,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/test-001/recipe.json
echo '{"id":"ss-001","type":"blueprint","topic":"API","phase":"cancelled","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/ss-001/recipe.json

# 21. Session-start — roadmap exists, no active recipe → roadmap hint
printf '# Roadmap\n\nStatus: CONFIRMED\nProgress: 1/3\n\n## Items\n\n- [x] Core models (recipe: test-001)\n- [ ] API endpoints (recipe: rm-001)\n- [ ] UI components\n' > .forge/state/roadmap.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"session_start"}' | $FORGE hook session-start 2>&1)
...
mkdir -p .forge/state/recipes/rm-001
echo '{"id":"rm-001","type":"blueprint","topic":"API endpoints","phase":"status","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/rm-001/recipe.json
echo '{"recipe_id":"rm-001","started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z","tasks":[{"id":"t-001","file":"src/api.ts","action":"create","status":"done","description":"api","depends_on":[],"retry_count":0,"last_error":""}]}' > .forge/state/recipes/rm-001/tasks.json
echo '{"recipe_id":"rm-001","run_at":"2026-03-18T00:00:00Z","framework":"jest","iterations":1,"status":"pass","total":3,"passed":3,"failed":0,"skipped":0}' > .forge/state/recipes/rm-001/test-results.json
echo "# Code Review" > .forge/state/recipes/rm-001/review.md
echo "# Deviation Report" > .forge/state/recipes/rm-001/deviation.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"stop","content":"<forge>IMPLEMENT DONE</forge>"}' | $FORGE hook stop 2>&1; echo "EXIT:$?")
...
grep -q '\[x\] API endpoints' .forge/state/roadmap.md && echo "✓ 24. roadmap item marked done" || { echo "✗ 24. roadmap mark done"; exit 1; }
...
printf '# Vision\n\nStatus: DRAFT\n' > .forge/state/vision.md
rm .forge/state/roadmap.md
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"session_start"}' | $FORGE hook session-start 2>&1)
...
mkdir -p .forge/state/recipes/ptu-001
echo '{"id":"ptu-001","type":"blueprint","topic":"Test","phase":"draft","iteration":1,"level":1.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/ptu-001/recipe.json
...
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"pre-tool-use","tool_name":"Write","tool_input":{"file_path":".forge/state/recipes/ptu-001/draft.md","content":"spec"}}' | $FORGE hook pre-tool-use 2>&1)
...
echo '{"id":"ptu-001","type":"blueprint","topic":"Test","phase":"implement","iteration":1,"level":3.0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/ptu-001/recipe.json
RESULT=$(echo '{"session_id":"t","cwd":"'"$TEST_DIR"'","hook_event_name":"pre-tool-use","tool_name":"Write","tool_input":{"file_path":"src/app.ts","content":"code"}}' | $FORGE hook pre-tool-use 2>&1)
...
mkdir -p .forge/state/recipes/disc-001
echo '{"id":"disc-001","type":"blueprint","topic":"Test","phase":"discovery","iteration":0,"level":0,"started_at":"2026-03-18T00:00:00Z","updated_at":"2026-03-18T00:00:00Z"}' > .forge/state/recipes/disc-001/recipe.json
