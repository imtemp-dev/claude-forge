...
```
大まかな計画 → AIコーディング → バグ → 修正 → バグ → 修正 → ...（N回繰り返し）
```

AI生成コードのデバッグに大部分の時間が費やされます。根本原因：仕様が曖昧で、AIが推測したためです。

## 解決策

```
仕様 → 検証 → 修正 → 検証 → ... → 完璧な仕様 → AIコーディング → 完了
```

コードではなく**ドキュメント**を反復します。ドキュメント変更はコストゼロ——ビルド、テスト、副作用なし。仕様が完璧であれば、AIは最小限の反復でコードを生成します。
